export function Hashtable() {
    var j = { __indexToValue: [], __indexToKeys: [] };
    var f = [];
    var h = 0;
    var k = this;
    function g(b) {
...
export function DomDataCollection(n) {
    var j = this;
    j.config = {
        recursion_level: 1,
    ...
    var startInspection1 = (j.startInspection = function() {
        var b = false;
        var c = true;
        BrowserDetect.init();
    ...
    var domDataAsJSON1 = (j.domDataAsJSON = function() {
    ...
    var l = new Hashtable();
    var initFunctionsToExclude1 = (j.initFunctionsToExclude = function() {
        if (l) {
            l.clear();
        }
        const a = j.config.functionsToExclude;
        var b = a.length;
        while (b--) {
            l.put(a[b], "");
        }
    ...
    return { domDataAsJSON1, initFunctionsToExclude1, startInspection1 };
}
...
ProxyCollector.isTimedOut = function(d, e, f) {
    ...
    if (
        e - _timer.duration() <= 0 &&
        ((typeof ProxyCollector.internalPingTime === "undefined" && new RegExp("internalPingTime").test(d)) ||
            (typeof ProxyCollector.externalPingTime === "undefined" && new RegExp("externalPingTime").test(d)))
    ) {
        d.call(this, -1);
        f.abort();
    } else {
    ...
    if (window.XDomainRequest) {
        ...
        try {
            m.onreadystatechange = function() {
                if (m.readyState == 4) {
                    if (
                        (typeof ProxyCollector.internalPingTime === "undefined" && new RegExp("internalPingTime").test(l)) ||
                        (typeof ProxyCollector.externalPingTime === "undefined" && new RegExp("externalPingTime").test(l))
                    ) {
                        l.call(this, o.duration());
                    }
                }
...
export function encode_deviceprint() {
...
export function post_deviceprint() {
    console.log("encode_deviceprint---->" + encode_deviceprint());
    return true;
}
...
function add_deviceprint() {
    ...
    var c =
        "version=" +
        d.deviceprint_version() +
        "&pm_fpua=" +
        d.deviceprint_browser() +
        "&pm_fpsc=" +
        d.deviceprint_display() +
        "&pm_fpsw=" +
        d.deviceprint_software() +
        "&pm_fptz=" +
        d.deviceprint_timezone() +
        "&pm_fpln=" +
        d.deviceprint_language() +
        "&pm_fpjv=" +
        d.deviceprint_java() +
        "&pm_fpco=" +
    ...
    c =
        c +
        "&pm_fpan=" +
        d.deviceprint_appName() +
        "&pm_fpacn=" +
        d.deviceprint_appCodeName() +
        "&pm_fpol=" +
        d.deviceprint_online() +
        "&pm_fposp=" +
        d.deviceprint_opsProfile() +
        "&pm_fpup=" +
        d.deviceprint_userProfile() +
        "&pm_fpsaw=" +
        d.deviceprint_screen_availWidth() +
        "&pm_fpspd=" +
        d.deviceprint_screen_pixelDepth() +
        "&pm_fpsbd=" +
        d.deviceprint_screen_bufferDepth() +
        "&pm_fpsdx=" +
        d.deviceprint_screen_deviceXDPI() +
        "&pm_fpsdy=" +
        d.deviceprint_screen_deviceYDPI() +
        "&pm_fpslx=" +
        d.deviceprint_screen_logicalXDPI() +
        "&pm_fpsly=" +
        d.deviceprint_screen_logicalYDPI() +
        "&pm_fpsfse=" +
        d.deviceprint_screen_fontSmoothingEnabled() +
        "&pm_fpsui=" +
...
var GEO_LOCATION_DEFAULT_STRUCT = '{"GeoLocationInfo":[{"PersonalSummaryStatus":4}]}';
var geoLocator = null;
...
const UIEventCollector = (function() {
    var K = null;
    var N = null;
    var Y = null;
    var M = null;
...
var BrowserDetect = {
    ...
    dataBrowser: [
        ...
        { string: navigator.userAgent.toLowerCase(), subString: "opera", identity: "Opera", versionSearch: "version" },
        { string: navigator.vendor, subString: "Apple", identity: "Safari", versionSearch: "Version" },
        { string: navigator.userAgent, subString: "mobile safari", identity: "Mobile Safari", versionSearch: "mobile safari" },
        { string: navigator.vendor, subString: "iCab", identity: "iCab" },
        { string: navigator.vendor, subString: "KDE", identity: "Konqueror" },
        { string: navigator.userAgent, subString: "Firefox", identity: "Firefox" },
        { string: navigator.vendor, subString: "Camino", identity: "Camino" },
        { string: navigator.userAgent.toLocaleLowerCase(), subString: "blackberry", identity: "BlackBerry", versionSearch: "0/" },
