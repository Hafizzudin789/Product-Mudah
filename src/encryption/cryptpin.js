...
function setKeyString(val) {
    keyVal = val;
}

/*replacePinWithStar method replaces the original pin with * in all the PIN fields.
 */
function replacePinWithStar() {
    with (document.forms[0]) {
        if (document.getElementById("pin").value.length == 6 && document.getElementById("confirmPin").value.length == 6) {
            document.getElementById("pin").value = document.getElementById("pin").value.replace(/./g, "*");
            document.getElementById("confirmPin").value = document.getElementById("confirmPin").value.replace(/./g, "*");
        }
    }
    return true;
}
...
//do_encrypt is TRUE for encrypt, FALSE for decrypt
function do_tdes(key1, key2, inputpin, do_encrypt) {
